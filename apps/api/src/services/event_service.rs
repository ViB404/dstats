...
impl EventService {
    pub async fn create_event(
        ...
        let event = EventRepository::create(pool, bot_id, guild_id, event_type, payload).await?;

        Ok(event)
    }

    pub async fn get_bot_events(pool: &PgPool, bot_id: Uuid) -> AppResult<Vec<Event>> {
        let events = EventRepository::find_by_bot_id(pool, bot_id).await?;

        Ok(events)
    }

    pub async fn get_guild_events(pool: &PgPool, guild_id: Uuid) -> AppResult<Vec<Event>> {
        let events = EventRepository::find_by_guild_id(pool, guild_id).await?;
    ...
    pub async fn get_recent_events(
        ...
        let events = EventRepository::find_recent_by_bot_id(pool, bot_id, limit, offset).await?;
