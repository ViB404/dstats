...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-neutral/80 backdrop-blur-xl border-b border-white/10 shadow-sm h-20 flex items-center">
      <div className="flex items-center justify-between px-4 md:px-12 h-full max-w-7xl mx-auto w-full">
        ...
        <div className="hidden md:flex items-center gap-8">
          {links.map((link) => (
            <Link key={link.name} href={link.href} className="text-neutral-400 hover:text-white transition-colors text-sm font-medium">
              {link.name}
            </Link>
          ))}
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <Dialog.Root>
            ...
            <Dialog.Portal>
              ...
              <Dialog.Popup className="fixed right-0 top-0 h-full w-3/4 max-w-sm border-l border-white/10 z-50 p-6 flex flex-col gap-6 outline-none shadow-2xl origin-right transition-transform data-ending-style:translate-x-full data-starting-style:translate-x-full">
                ...
                <div className="flex flex-col gap-6 mt-4">
                  {links.map((link) => (
                    <Link key={link.name} href={link.href} className="text-lg font-medium text-neutral-200 hover:text-tertiary transition-colors">
                      {link.name}
                    </Link>
                  ))}
                  ...
                  <div className="mt-4 pt-4 border-t border-white/10">
                    {!isLoaded ? (
                      ...
                      <div className="flex items-center gap-4 p-2 rounded-lg bg-white/5 border border-white/10">
                        <UserButton />
                        <span className="text-sm font-medium text-neutral-200">Manage Account</span>
                      </div>
                    )}
                  </div>
                </div>
